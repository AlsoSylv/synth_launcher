...
pub async fn game_versions(client: &reqwest::Client) -> reqwest::Result<Full> {
    client.get(format!("{BASE}/v2/versions/game")).send().await?.json().await
}

pub async fn game_yarn_versions(client: &reqwest::Client) -> reqwest::Result<Full> {
    client.get(format!("{BASE}/v2/versions/game/yarn")).send().await?.json().await
}

pub async fn game_intermediary_versions(client: &reqwest::Client) -> reqwest::Result<Full> {
    client.get(format!("{BASE}/v2/versions/game/intermediary")).send().await?.json().await
}

pub async fn intermediary_versions(client: &reqwest::Client) -> reqwest::Result<Full> {
    client.get(format!("{BASE}/v2/versions/intermediary")).send().await?.json().await
}

pub async fn intermediary_versions_for_game_version(client: &reqwest::Client, game_version: &str) -> reqwest::Result<Full> {
